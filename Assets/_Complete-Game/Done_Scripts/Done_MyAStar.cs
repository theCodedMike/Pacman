...
public class Done_MyAStar : MonoBehaviour
{
	/// <summary>
	/// 单例脚本
	/// </summary>
	public static Done_MyAStar instance;
		...
		for (int i = 0; i < x; i++) {
			for (int j = 0; j < y; j++) {
				...
				GameObject item = (GameObject)Instantiate (reference, 
					                  ...
					                  Quaternion.identity);
				item.transform.GetChild (0).GetComponent<Done_Reference> ().x = i;
				item.transform.GetChild (0).GetComponent<Done_Reference> ().y = j;
				objs [i, j] = item;
	...
	IEnumerator ShowResult ()
	{
		...
		while (parentList.Count != 0) {
            ...
            //以颜色方式绘制路径
            Debug.Log("load");
            FindObjectOfType<Done_GhostMove>().ghostMove(x, y);
